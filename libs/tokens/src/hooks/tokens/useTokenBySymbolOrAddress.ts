...
import { tokensByAddressAtom, tokensBySymbolAtom, tokensBySymbolAtomByNetworkNumber } from '../../state/tokens/allTokensAtom'
...
export function useTokenBySymbolOrAddressByNetworkNumber(symbolOrAddress?: string | null): TokenWithLogo | null {
  const tokensByAddress = useAtomValue(tokensByAddressAtom)
  const tokensBySymbol = useAtomValue(tokensBySymbolAtomByNetworkNumber)
